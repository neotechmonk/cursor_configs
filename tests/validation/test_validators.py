...
def test_validate_identical_output_by_different_steps():
    """Test that validate_identical_output_by_different_steps correctly prevents different steps from producing identical outputs."""
    # Create two different steps
    step1 = StrategyStep(
        id="step1",
        name="Step One",
    ...
    # First step's output should be accepted
    validate_identical_output_by_different_steps(
        cur_step=step1,
    ...
    with pytest.raises(ValueError) as exc_info:
        validate_identical_output_by_different_steps(
            cur_step=step2,
            cur_step_result=result,
            prev_results={(None, step1): result}
        )
    
    # Verify error message
    assert "Steps 'Step Two' and 'Step One' produced identical output keys" in str(exc_info.value) 